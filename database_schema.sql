...
-- Enhancements for authentication: verification, last_login and profile
-- Add verification and last_login columns to users table if not present
alter table users add column if not exists is_verified boolean default false;
alter table users add column if not exists verification_code varchar(64) null;
alter table users add column if not exists verification_expiry datetime null;
alter table users add column if not exists last_login datetime null;

-- A lightweight user_profiles table to mirror Django-style profile data (optional)
